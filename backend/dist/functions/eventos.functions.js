...
exports.gastoTotal = (req, res) => {
    ...
    let total = 0;
    interfisca.consultarEventoFisca(alianza, persona, categoria, subcategoria, (docs) => {
        if (!docs) {
            res.send('');
        }
        for (let i of docs) {
...
exports.gastoSubcategorias = (req, res) => {
    let alianza = req.body['alianza'];
    let persona = req.body['persona'];
    let categoria = req.body['categoria'];
    let subcategorias;
    ...
    for (let subcatego in subcategorias) {
        interfisca.consultarEventoFisca(alianza, persona, categoria, subcatego, (docs) => {
            if (!docs) {
                res.send('');
            }
            for (let i of docs) {
...
//# sourceMappingURL=eventos.functions.js.map